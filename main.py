...
def load_data(_filename, n_steps=4, without=False):
...
def modeler(train, validate):
    ...
    # Train the model
    model, history = compiler(model, x_train, y_train, validate, x_validate, y_validate)
...
def statistics(station_name, y_test, y_pred):
    """
    Compute the statistics of the model
    :param station_name: the station_name used for the data
